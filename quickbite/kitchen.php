...
<head>
  ...
  <title>QuickBite - Kitchen Monitor</title>
...
<body>
  ...
  <div class="kitchen-hero">
    <div class="hero-content">
      <h1 class="hero-title" 
          data-lang-en="Kitchen Monitor"
          data-lang-ta="சமையலறை மானிட்டர்"
          data-lang-hi="रसोई मॉनिटर">
        Kitchen Monitor
      </h1>
    </div>
  </div>
    ...
    <div class="container kitchen-container">
      <div class="orders-container" id="ordersContainer">
        ...
        <div class="empty-state">
          ...
          <h3 data-lang-en="No pending orders" data-lang-ta="நிலுவையில் உள்ள ஆர்டர்கள் இல்லை" data-lang-hi="कोई लंबित आदेश नहीं">No pending orders</h3>
  ...
  <script>
    ...
    function startOrderPolling() {
      setInterval(renderKitchenOrders, 3000); // Check every 3 seconds
    }
    ...
    function renderKitchenOrders() {
      ...
      kitchenQueue.forEach(order => {
        const statusClass = order.kitchenStatus === 'preparing' ? 'status-preparing' : 'status-pending';
        const statusText = order.kitchenStatus === 'preparing' ? 
                          (currentLanguage === 'en' ? 'Preparing' : 
                           currentLanguage === 'ta' ? 'தயாராகிறது' : 'तैयार हो रहा है') : 
          ...
          <div class="order-card" data-order-id="${order.orderId}">
            ...
            <div class="order-actions">
              ${order.kitchenStatus !== 'preparing' ? `
                <button class="action-btn start-btn" data-order-id="${order.orderId}">
                  <i class="fas fa-utensils"></i>
                  <span data-lang-en="Start Preparing" data-lang-ta="தயாரிக்கத் தொடங்கு" data-lang-hi="तैयार करना शुरू करें">Start Preparing</span>
                </button>
              ` : `
                <button class="action-btn complete-btn" data-order-id="${order.orderId}">
                  <i class="fas fa-check"></i>
                  <span data-lang-en="Mark Complete" data-lang-ta="முடிந்ததாகக் குறிக்கவும்" data-lang-hi="पूर्ण चिह्नित करें">Mark Complete</span>
                </button>
              `}
            </div>
          </div>
    ...
    function updateOrderStatus(orderId, status) {
      const kitchenQueue = JSON.parse(localStorage.getItem('kitchenQueue')) || [];
      const orderIndex = kitchenQueue.findIndex(o => o.orderId === orderId);
      
      if (orderIndex !== -1) {
        kitchenQueue[orderIndex].kitchenStatus = status;
        kitchenQueue[orderIndex].prepStartTime = new Date().toISOString();
    ...
    function completeOrder(orderId) {
      ...
      const completedOrder = kitchenQueue.find(o => o.orderId === orderId);
      kitchenQueue = kitchenQueue.filter(o => o.orderId !== orderId);
      localStorage.setItem('kitchenQueue', JSON.stringify(kitchenQueue));
      ...
      if (orderIndex !== -1) {
        billingQueue[orderIndex].kitchenStatus = 'completed';
        billingQueue[orderIndex].prepEndTime = new Date().toISOString();
        localStorage.setItem('billingQueue', JSON.stringify(billingQueue));
      } else if (completedOrder) {
        completedOrder.kitchenStatus = 'completed';
        completedOrder.prepEndTime = new Date().toISOString();
        billingQueue.push(completedOrder);
        localStorage.setItem('billingQueue', JSON.stringify(billingQueue));
      }
      ...
      const currentOrder = JSON.parse(localStorage.getItem('currentOrder'));
      if (currentOrder && currentOrder.orderId === orderId) {
        currentOrder.kitchenStatus = 'completed';
        localStorage.setItem('currentOrder', JSON.stringify(currentOrder));
      }
